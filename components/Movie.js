...
function Movie(props) {
  ...
  for (let i = 0; i < 10; i++) {
    personalStars.push(
      <FontAwesomeIcon
        ...
        style={i < personalNote ? { color: '#2196f3', cursor: 'pointer' } : { cursor: 'pointer' }}
      />
    );
  }
  ...
  const videoStyle = watchCount > 0 ? { color: '#e74c3c', cursor: 'pointer' } : { cursor: 'pointer' };

  // FAVORIS
  const handleLike = () => props.updateLikedMovies(props.title);
  const heartStyle = props.isLiked ? { color: '#e74c3c', cursor: 'pointer' } : { cursor: 'pointer' };
